# Hi 👋 I'm Mahmoud Ghanem

DevOps Engineer focused on building cloud-native infrastructure, Kubernetes platforms, and automated CI/CD pipelines.

I enjoy designing scalable systems using Kubernetes, AWS, and GitOps practices.

---

# 🚀 DevOps Stack

![AWS](https://img.shields.io/badge/AWS-FF9900?logo=amazonaws&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?logo=kubernetes&logoColor=white)
![ArgoCD](https://img.shields.io/badge/ArgoCD-EF7B4D?logo=argo&logoColor=white)
![Prometheus](https://img.shields.io/badge/Prometheus-E6522C?logo=prometheus&logoColor=white)
![Grafana](https://img.shields.io/badge/Grafana-F46800?logo=grafana&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHubActions-2088FF?logo=githubactions&logoColor=white)

---

# 📦 Featured Projects

## DevOps GitOps on AWS EKS

End-to-end DevOps pipeline deploying a Spring Boot application to AWS EKS using GitOps.

### Architecture

GitHub → GitHub Actions → DockerHub → ArgoCD → Kubernetes (EKS) → Spring Boot App → Prometheus → Grafana

### Technologies Used

- AWS EKS
- Docker
- GitHub Actions
- ArgoCD
- Prometheus
- Grafana
- AWS Application Load Balancer

### Repository

👉 Repository: https://github.com/mahmoudghanem1/eks-gitops-pipeline
---

## Kubernetes GitOps Deployment

GitOps CI/CD pipeline deploying a Spring Boot application to Kubernetes with monitoring and alerting.

### Architecture

GitHub → GitHub Actions → DockerHub → ArgoCD → Kubernetes → Spring Boot App → Prometheus → Grafana

### Technologies Used

- Kubernetes
- Docker
- GitHub Actions
- ArgoCD
- Prometheus
- Grafana

### Repository

👉 Repository: https://github.com/mahmoudghanem1/gitops-k8s-pipeline

---

# 🧱 DevOps Skills

- CI/CD Pipelines
- Docker Containerization
- Kubernetes Deployment
- GitOps with ArgoCD
- Monitoring with Prometheus & Grafana
- AWS Cloud Infrastructure
- Infrastructure Automation

---

# 🌍 Connect with Me

LinkedIn  : www.linkedin.com/in/proghanem
Email  : pro.mahmoudghanem@gmail.com

---

⭐ Always learning and building DevOps systems.
